...
### Introduction:
The project was started to get first insights into Excel VBA - some 'real life data' and the plan to visualize weather balloon flights. 
...
Just download the Excel file, get an idea how it works, feel free to improve and adjust the macro or the charts to suite your needs. Most of the steps in the code have comments or links to sources.
...
### Let's get started:
Download the latest Excel file (root directory) and some CSV example (sub-folder). Later you can download more CSVs via the macro or via [radiosondy.info](https://radiosondy.info). The CSV examples are from chases we participated successfully by ourselves or together with other seekers. The unlucky chases are not included... ;-)
...
For running the macro, please ensure Excel can execute it. You may get one of those warnings, which you can accept.
...
Implausible values in 'description' strings are ignored but can cause glitches during parsing, like this example:
